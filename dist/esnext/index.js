 ...
 */
const forEach = (iteratee, enumerable) => {
    let index = 0;
 ...
 */
export const toArray = (enumerable) => {
    const collection = [];
    forEach((item) => {
 ...
 */
const some = (iteratee, enumerable) => {
    let val = false;
    forEach((item, i) => {
 ...
 * @returns {boolean} true if the callback
 */
const every = (iteratee, enumerable) => {
    let val = true;
    let hasitems = false;
    forEach((item, i) => {
        hasitems = true;
 ...
 * @returns {boolean} true if the callback
 */
const find = (iteratee, enumerable) => {
    let val;
    forEach((item, i) => {
        if (iteratee(item, i, enumerable)) {
            val = item;
 ...
 */
const reduce = (iteratee, accumulator, enumerable) => {
    forEach((item, i) => {
 ...
 */
const groupBy = (iteratee, enumerable) => {
...
//     { [group: string]: any[] }
// >(groupByF);
const matches = (source, enumerable) => {
    return item => {
 ...
 * @param enumerable
 */
const filter = (predicate, enumerable) => {
 ...
 */
const firstOrDefault = (iteratee, enumerable) => {
    let index = 0;
    const enumerator = enumerable.getEnumerator();
    if (enumerator.moveNext()) {
        const current = enumerator.get_current();
        if (iteratee) {
            if (iteratee(current, index++, enumerable))
                return current;
        }
        else
            return current;
 ...
 */
const executeQuery$ = (context, data) => {
    const deferred = (NWF$ || $).Deferred();
    context.executeQueryAsync((sender, args) => deferred.resolve(data, args, sender), (sender, args) => deferred.reject(args, sender));
 ...
 */
const getQueryResult = (queryText, list) => {
